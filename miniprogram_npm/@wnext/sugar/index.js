...
__DEFINE__(1600743899313, function(require, module, exports) {


Object.defineProperty(exports, '__esModule', { value: true });

var lifetimeMap = new Map();
/**
 * 追踪生命周期函数到集合
 *
 * @param name
 */
function trackLifetime(name) {
    return function lifetime(fn) {
        if (!lifetimeMap.has(name)) {
            lifetimeMap.set(name, new Set());
        }
        var lifetimeSet = lifetimeMap.get(name);
        if (lifetimeSet) {
            lifetimeSet.add(fn);
        }
    };
}
var onLoad = trackLifetime('onLoad');
var onShow = trackLifetime('onShow');
...
function trigger$1(target, type, key, newValue, oldValue, oldTarget) {
    ...
    const add = (effectsToAdd) => {
        if (effectsToAdd) {
            effectsToAdd.forEach(effect => {
                if (effect !== activeEffect$1 || effect.options.allowRecurse) {
                    effects.add(effect);
                }
            });
        }
...
function createGetter$1(isReadonly = false, shallow = false) {
    return function get(target, key, receiver) {
        ...
        const res = Reflect.get(target, key, receiver);
        const keyIsSymbol = isSymbol$1(key);
        if (keyIsSymbol
...
function ownKeys$1(target) {
    track$1(target, "iterate" /* ITERATE */, ITERATE_KEY$1);
    return Reflect.ownKeys(target);
}
...
function setupRenderEffect(instance) {
    instance.update = effect$1(function () {
        if (instance && instance.render) {
            var a = instance.render;
            console.log(a);
        }
    });
}
...
return __REQUIRE__(1600743899313);
})()
